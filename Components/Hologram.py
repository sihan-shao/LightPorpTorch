...
class HologramLayer(ABC, nn.Module):
    ...
    def modulate(self, 
        ...
        if input_field.height != preprocessed_height_map.shape[0] or input_field.width != preprocessed_height_map.shape[1]:
            ...
            preprocessed_height_map = torch.squeeze(preprocessed_height_map, (0, 1))
        # --------------------------------------- #
...
class FullPrecisionHologramLayer(HologramLayer):
    def __init__(self, 
                 holo_size      : list = None,
                 height_constraint_max: float = None,
                 tolerance      : float = 0.1*mm,
    ...
    def preprocessed_height_map(self):
        self.height_map = self.height_constraint_max * torch.sigmoid(torch.clamp(self.weight_height_map, min=-10.0, max=10.0))
        return self.height_map
...
class PSQuantizedHologramLayer(HologramLayer):
    # Progressive Sigmoid Quantization (PSQ) method
    def __init__(self, 
                 holo_size      : list = None,
                 holo_level     : int = 6,
                 num_unit       : int = None,
                 height_constraint_min: float = 0,
                 height_constraint_max: float = None,
                 tolerance      : float = 0.1*mm,
                 material       : list = None,
                 circ_aperture  : bool = True,
        ...
        self.height_constraint_min = torch.tensor(height_constraint_min, device=self.device)
        self.height_constraint_max = torch.tensor(height_constraint_max, device=self.device)
        self.tolerance      = tolerance
    ...
    def build_weight_height_map(self):
        ...
        if self.num_unit is None:
            self.weight_height_map = nn.parameter.Parameter(
                self.height_constraint_max * torch.rand(height, width, device=self.device), requires_grad=True)
        else:
            unit_size = [int(height / self.num_unit), int(width / self.num_unit)]
            self.weight_height_map = nn.parameter.Parameter(
                self.height_constraint_max * torch.rand(unit_size[0], unit_size[1], device=self.device), requires_grad=True
            )
    ...
    def preprocessed_height_map(self, tau):
        ...
        delta = (self.height_constraint_max - self.height_constraint_min) / (self.holo_level - 1)
        
        height_map_clamped = torch.clamp(self.weight_height_map, self.height_constraint_min, self.height_constraint_max)
        x_normalized = (height_map_clamped - self.height_constraint_min) / delta - 0.5
        levels_range = torch.arange(self.holo_level - 1, device=self.weight_height_map.device).unsqueeze(0).unsqueeze(2)

        self.height_map = self.height_constraint_min + delta * torch.sum(
    ...
    def forward(self, field: ElectricField, iter_frac=None)->ElectricField:
        ...
        def tau_iter(iter_frac, tau_min=1, tau_max=50, r=None):
            if r is None:
                r = math.log(tau_max / tau_min)
            tau = tau_min *math.exp(r * iter_frac)
            return tau
